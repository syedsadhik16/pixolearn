...
export function PerformanceAnalytics({ child }: Props) {
  ...
  const dailyData = useMemo(() => {
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      const dc = child.completions.filter(c => format(new Date(c.completed_at), 'yyyy-MM-dd') === d);
      days.push({ date: format(new Date(d), 'MMM d'), score: getAvg(dc.flatMap(c => [c.pronunciation_score, c.fluency_score, c.clarity_score, c.confidence_score])), lessons: dc.length });
    }
    return days;
  }, [child]);
  ...
  const weeklyData = useMemo(() => {
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      const we = subDays(new Date(), (i - 1) * 7);
      const wc = child.completions.filter(c => { const d = new Date(c.completed_at); return d >= ws && d < we; });
      weeks.push({ date: format(ws, 'MMM d'), score: getAvg(wc.flatMap(c => [c.pronunciation_score, c.fluency_score, c.clarity_score, c.confidence_score])), lessons: wc.length });
    }
    return weeks;
  }, [child]);
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const mc = child.completions.filter(c => { const d = new Date(c.completed_at); return d >= ms && d < me; });
      months.push({ date: format(ms, 'MMM'), score: getAvg(mc.flatMap(c => [c.pronunciation_score, c.fluency_score, c.clarity_score, c.confidence_score])), lessons: mc.length });
    }
    return months;
  }, [child]);
  ...
  const currentAvg = getAvg(recentCompletions.flatMap(c => [c.pronunciation_score, c.fluency_score, c.clarity_score, c.confidence_score]));
  ...
  const tooltipStyle = { backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '0.5rem', fontSize: '0.8rem' };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <CardContent className="space-y-3">
          {skills.map(skill => (
            <div key={skill.name} className="space-y-1">
              <div className="flex justify-between text-sm"><span>{skill.name}</span><span className="font-bold">{skill.score}%</span></div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-2xl font-bold">{child.completions.length}</p><p className="text-xs text-muted-foreground">Lessons Done</p></div>
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-2xl font-bold">{child.writingSubmissions.length}</p><p className="text-xs text-muted-foreground">Writings</p></div>
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-2xl font-bold">{currentAvg}%</p><p className="text-xs text-muted-foreground">Accuracy</p></div>
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-2xl font-bold">{child.streak}</p><p className="text-xs text-muted-foreground">Streak Days</p></div>
          </div>
        </CardContent>
      </Card>
    </div>
